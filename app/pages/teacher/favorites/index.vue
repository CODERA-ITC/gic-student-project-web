    ...
    <UContainer class="py-12">
      ...
      <div class="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 p-6 mb-8">
        <div class="flex items-center justify-between">
          ...
          <div v-if="favoriteProjects.length > 0" class="flex items-center gap-2">
            <ButtonsPresetButton label="Export List" icon="i-heroicons-arrow-down-tray" color="secondary"
              variant="outline" size="sm" @click="exportFavorites" />
      ...
      <!-- Empty State -->
      <div v-if="favoriteProjects.length === 0" class="flex flex-col items-center justify-center py-20">
        <UIcon name="i-heroicons-heart" class="w-24 h-24 text-gray-300 dark:text-gray-600 mb-4" />
        ...
        <ButtonsPresetButton label="Browse Projects" icon="i-heroicons-rocket-launch" color="primary" variant="solid"
          size="sm" @click="$router.push('/projects')" />
      </div>

      <!-- Projects Grid -->
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
        <ProjectCard v-for="project in filteredProjects" :key="project.id" :project="project"
          :liked-projects="projectStore.likedProjects" @toggle-like="toggleLike" />
      </div>
    </UContainer>

    <!-- Clear Confirmation Modal -->
    <DeleteConfirmationModal v-model="showClearConfirm" projectTitle="all favorite projects"
...
const exportFavorites = () => {
  ...
  a.href = url;
  a.download = `favorite-projects-${new Date().toISOString().split("T")[0]
    }.csv`;
  a.click();
