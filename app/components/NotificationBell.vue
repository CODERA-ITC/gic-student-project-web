<template>
  <div class="relative" ref="buttonRef">
    ...
    <button
      ...
      :aria-label="`${unreadCount} notifications`" @click="dropdownOpen = !dropdownOpen">
    ...
    <Transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
      ...
      <div v-show="dropdownOpen"
        class="absolute right-0 mt-2 w-[480px] bg-white dark:bg-gray-800 rounded-lg shadow-xl z-50 border border-gray-200 dark:border-gray-700">
        <!-- Header -->
        <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
          <div class="flex items-center justify-between">
            ...
            <button class="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
              ...
              <UIcon name="i-heroicons-cog-6-tooth" class="w-5 h-5 text-gray-600 dark:text-gray-400" />
            </button>
          </div>
        </div>
        ...
        <div class="max-h-[600px] overflow-y-auto bg-white dark:bg-gray-800">
          <div v-if="notifications.length === 0" class="px-4 py-12 text-center text-gray-500 dark:text-gray-400">
            <UIcon name="i-heroicons-inbox-20-solid" class="w-12 h-12 mx-auto mb-3 text-gray-400 dark:text-gray-500" />
          ...
          <button v-for="notification in notifications" :key="notification.id"
            ...
            <div class="flex gap-3">
              ...
              <div class="flex-shrink-0">
                <img v-if="notification.avatar" :src="notification.avatar" :alt="notification.channel"
              ...
              <div class="flex-1 min-w-0">
                <p class="text-sm text-gray-900 dark:text-gray-100 line-clamp-2 leading-tight mb-1">
                  <span v-if="notification.channel" class="font-medium">{{ notification.channel }}</span>
                  {{ notification.channel ? ' uploaded: ' : '' }}{{ notification.title }}
                </p>
              ...
              <div v-if="notification.thumbnail" class="flex-shrink-0">
                <img :src="notification.thumbnail" alt="Thumbnail" class="w-[120px] h-[68px] object-cover rounded" />
              </div>
...
watchEffect(() => {
  console.log('🔔 NotificationBell received notifications:', props.notifications);
  console.log('🔔 NotificationBell notifications count:', props.notifications.length);
});
...
const getNotificationIconClass = (type: string): string => {
  const classMap: Record<string, string> = {
    ...
    comment: "bg-indigo-100 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400",
  };
