    ...
    <div class="space-y-6">
        ...
        <form @submit.prevent="handleVerify" class="space-y-4">
            ...
            <div v-for="(question, index) in securityQuestions" :key="index">
                ...
                <input v-model="answers[index]" type="text" required :disabled="isLoading"
            ...
            <ButtonsPresetButton preset="primary" label="VERIFY ANSWERS" :loading="isLoading"
                :disabled="isLoading || !allAnswersFilled" size="lg" class="w-full" type="submit" />
        </form>

        <!-- Back Link -->
        <div class="text-center">
            <button type="button" @click="$emit('back')" :disabled="isLoading"
...
const handleVerify = async () => {
    ...
    try {
        ...
        if (!response.ok) {
            attemptsRemaining.value = data.attemptsRemaining || 0;
