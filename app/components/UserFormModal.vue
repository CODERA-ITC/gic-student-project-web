  ...
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="close">
        ...
        <div
          class="relative w-full max-w-2xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">
          <form class="p-6 space-y-5" @submit.prevent="submit" aria-label="User form">
            <div class="flex items-center justify-between">
              ...
              <ButtonsPresetButton label="" icon="i-heroicons-x-mark" color="secondary" variant="ghost" size="sm"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div class="sm:col-span-2">
                ...
                <div class="flex flex-wrap gap-2">
                  <button v-for="role in effectiveRoleOptions" :key="role" type="button" :class="[
                    'px-3 py-2 min-h-[44px] rounded-3xl border text-sm font-semibold transition-colors',
                    formState.role === role
                  ...
                  ]" :disabled="!!props.fixedRole" @click="setRole(role)">
                    {{ role }}
                  </button>
                </div>
              </div>
              ...
              <UFormGroup :label="passwordLabel">
                ...
                <div class="mt-2 flex flex-row gap-2 ">
                  <ButtonsPresetButton label="Auto Generate (8)" type="button" size="sm" color="primary"
                    ...
                    class="!text-blue-900 !border-blue-900 hover:!bg-blue-50" @click="generatePassword" />
                  <ButtonsPresetButton label="Copy" type="button" size="sm" color="secondary" variant="outline"
                    icon="i-heroicons-clipboard-document" :disabled="!formState.password" @click="copyPassword" />
                </div>
              </UFormGroup>
            ...
            <UFormGroup label="Bio (optional)">
              <textarea v-model="formState.bio" rows="3"
                class="w-full px-3 py-2 min-h-[44px] rounded-3xl border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                placeholder="Brief description" />
            </UFormGroup>

            <div class="flex justify-end gap-3 pt-2">
              <ButtonsPresetButton label="Cancel" size="sm" variant="ghost" color="secondary" type="button"
                @click="close" />
              <ButtonsPresetButton :label="actionLabel" size="sm" color="primary" type="submit" :loading="loading"
                :disabled="loading || isDisabled" class="!bg-blue-900 !text-white hover:!bg-blue-800" />
            </div>
          </form>
        </div>
      </div>
