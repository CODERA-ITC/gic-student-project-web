  ...
  <nav
    ...
    <div class="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
      ...
      <NuxtLink to="/" class="flex items-center shrink-0 space-x-2">
        <NuxtImg src="/images/itc-logo.png" alt="ITC Logo" width="auto" height="54"
        ...
        <NuxtImg src="/images/gic-logo-small.png" alt="GIC Showcase" width="auto" height="47"
          class="object-contain hover:opacity-80 transition-opacity duration-200 rounded-xs" />
      </NuxtLink>

      <!-- Desktop Navigation -->
      <div class="hidden lg:flex items-center justify-center space-x-1 flex-1 mx-8">
        <NavLink v-for="item in navigationItems" :key="item.to" :to="item.allow === false ? '' : item.to"
          :label="item.label" :is-active="isActiveRoute(item.to)" :badge="item.badge" />
      </div>

      <!-- Right Section: Language Switcher, Theme Toggle & User Menu -->
      <div class="flex items-center gap-2">
        <!-- Search Icon (Always Visible) -->
        <SearchBar :context="searchContext" @search="handleSearch" @clear="clearSearch" />
        ...
        <div class="hidden lg:block">
          ...
          <div v-if="authStore.isLoading || isInitialLoad"
            class="w-10 h-10 rounded-full bg-gray-200 dark:bg-neutral-700 animate-pulse"></div>
          <UserMenu v-else :is-authenticated="isAuthenticated" :user="user" @logout="handleLogout" />
        </div>

        <!-- Mobile Menu Button -->
        <UButton :icon="mobileMenuOpen
          ? 'i-heroicons-x-mark-20-solid'
          : 'i-heroicons-bars-3-20-solid'
          " color="gray" variant="ghost" size="md" class="lg:hidden text-neutral-700 dark:text-neutral-200"
          :ui="{ rounded: 'rounded-lg' }" @click="mobileMenuOpen = !mobileMenuOpen" />
      </div>
    </div>
    ...
    <Transition enter-active-class="transition ease-out duration-100"
      ...
      <div v-if="mobileMenuOpen"
        ...
        <div v-if="isAuthenticated" class="py-4 border-b border-gray-200 dark:border-neutral-800">
        ...
        <div class="py-4 space-y-2">
          ...
          <NavLink v-for="item in navigationItems" :key="item.to" :to="item.to" :label="item.label"
            :is-active="isActiveRoute(item.to)" :badge="item.badge" mobile mobile-class="w-full px-4 py-3 rounded-lg"
          ...
          <template v-if="isAuthenticated">
            ...
            <template v-if="authStore.isStudent">
              ...
              <NuxtLink to="/student/my-projects"
                class="flex items-center gap-2 px-4 py-3 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-800 transition-colors text-neutral-700 dark:text-neutral-200"
                @click="mobileMenuOpen = false">
                <UIcon name="i-heroicons-folder-20-solid" />
                {{ t("nav.myProjects") }}
              </NuxtLink>
              <NuxtLink to="/student/favorites"
                class="flex items-center gap-2 px-4 py-3 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-800 transition-colors text-neutral-700 dark:text-neutral-200"
                @click="mobileMenuOpen = false">
          ...
          <template v-else>
            <ButtonsPresetButton preset="signin" to="/login" class="w-full" size="md" @click="mobileMenuOpen = false" />
          </template>
        </div>
      </div>
    </Transition>
