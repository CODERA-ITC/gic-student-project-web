  ...
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="closeModal">
        <!-- Backdrop -->
        <div class="absolute inset-0 bg-gray-900/75 dark:bg-gray-900/90 backdrop-blur-sm" @click="closeModal"></div>
        ...
        <div
          ...
          <div class="flex flex-col items-center gap-3">
            <div
              ...
              <UIcon name="i-heroicons-exclamation-triangle" class="w-7 h-7 text-red-600 dark:text-red-300" />
            </div>
          ...
          <div class="flex flex-col sm:flex-row gap-3">
            <ButtonsPresetButton preset="cancel" size="sm" @click="closeModal" />
            <ButtonsPresetButton preset="delete" :disabled="deleteText !== 'DELETE'" :loading="isDeleting"
              @click="handleConfirm" />
          </div>
        </div>
      </div>
