<template>
  ...
  >
    ...
    >
      ...
      <div class="mb-6">
        <h2
          class="text-2xl font-semibold text-slate-900 dark:text-white mb-2 flex items-center gap-2"
        >
          <UIcon
            name="i-heroicons-shield-check"
            class="w-7 h-7 text-blue-900 dark:text-blue-400"
          />
      ...
      <div v-if="loadingQuestions" class="flex justify-center py-12">
        <div
          class="w-12 h-12 border-4 border-blue-900 border-t-transparent rounded-full animate-spin"
        ></div>
      </div>
...
const handleSubmit = async () => {
  ...
  if (!isFormValid.value) {
    error.value =
      "Please answer all security questions (2-100 characters each)";
    return;
  }
