export default defineEventHandler(async (event): Promise<void> => {
  try {
    ...
    const response: any = await $fetch(
      `${API_BASE_URL}/users/github/callback?code=${code}`,
      {
        method: "GET",
      }
    );
    ...
    if (response && typeof response === "object") {
      ...
      if (access_token) {
        const redirectUrl: string = `/auth/callback?token=${access_token}${
    ...
    return sendRedirect(
      event,
      "/login?error=Invalid response from authentication server"
    );
  } catch (error) {
